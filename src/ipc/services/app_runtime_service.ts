...
/**
 * Detect whether the app at the given path is a Next.js project.
 *
 * Used to decide whether we can safely append `--webpack` to the dev command:
 * Next.js (15.3+) accepts `--webpack` to disable Turbopack, but Vite (and other
 * CAC-based dev servers) reject unknown flags and crash on startup.
 */
...
async function getDefaultCommand({
  ...
  if (runtimeMode === "docker") {
    ...
    return `${getPnpmInstallCommand()} && pnpm run dev --port ${port}${turbopackOverride}`;
  }

  const pnpmSupport = await getPnpmMinimumReleaseAgeSupport();
  const npmCommand = `(${getNpmInstallCommand()} && npm run dev -- --port ${port}${turbopackOverride})`;
